...
RegisterNUICallback('getCaseTimeline', function(payload, cb)
    local rows = lib.callback.await('doj_finance_suite:server:getCaseTimeline', false, payload.source_type, payload.source_id, payload.source_key)
    cb({ rows = rows or {} })
end)
...
RegisterNUICallback('setCaseMeta', function(payload, cb)
    local ok = lib.callback.await('doj_finance_suite:server:setCaseMeta', false, payload)
    cb({ ok = ok and true or false })
end)
...
RegisterNUICallback('getBusinessLinkProfile', function(payload, cb)
    local profile = lib.callback.await('doj_finance_suite:server:getBusinessLinkProfile', false, payload and payload.business_id or '')
    cb(profile or {})
end)
